...
> **2026-08-08：编号计划的正文（001–031）已删除，当时本文件成为该批计划的唯一保留物（后续新增正文仍保留）。**
> 31 份正文里共 218 个未勾选项，且 31/31 都以「Executor instructions / STOP conditions」
> 这类祈使句开头——正是本文件早就记录过的那个「读到就可能被带偏」的常设面。
> 而它们描述的工作已全部落地或被实测驳回，实现就在代码里，状态与理由在下面的表格
> 和各次复核块中。需要回看正文用 `git log -- plans/`。
>
> **删除时机的判断**：等到「已无待执行计划」才删。此前正文还承载着执行指令，
> 删了会让在途工作失去依据；现在它们只剩历史，留着的唯一效果是被误当成 backlog。
...
**2026-08-09：产品口径变更——任务的「预计番茄数」改为「预计用时」（分钟）。**
schema 升到 v10：`tasks.estimated_pomodoros` → `estimated_minutes`，迁移按 `×25` 回填后
删除旧列。自动完成任务的判据随之从「番茄数达标」变成「累计专注时长达标」，且不再要求
该次会话计为有效番茄——自由计时也算进预计用时。周复盘的「计划」维度同步改为分钟
（`plannedMinutes`），科目级参与门槛从「计划 ≥3 个番茄」变成「计划 ≥60 分钟」。
UI 侧新增共享组件 `qml/components/DurationFieldPair.qml`（小时+分钟输入对），
今日专注目标与任务预计用时共用同一份实现与越界规则；分钟展示统一走 `qml/Duration.js`。
**下面第 299/309/719 行提到的 `estimated_pomodoros` 是当时的列名，现已不存在。**

**2026-08-10：用户视角功能审核，六项缺口全部落地（schema v11）。**
审核的问法是「一个考研用户会真的撞上什么」，而不是「还能加什么功能」。
结论里最重的一条是**长期目标其实一直漏计**：它按「完整番茄数」算进度，
而任务侧早已改成分钟且把自由计时算在内——同一段专注在两处得出相反结论，
用自由计时刷三小时，任务进度涨了、长期目标纹丝不动。这不是单位不一致，是缺陷。

v11 一次改三处（合并成一次迁移只建一份快照）：
`long_goals.target_pomodoros` → `target_minutes`（×25 回填，与 v10 同一基准，
否则同一用户的任务预估和目标预估会按两把尺子折算）；`tasks.notes`；`tasks.display_order`。

六项各自的落点：
1. 长期目标改分钟，进度口径换成 `FocusSessionRules::focusedSecondsExpr`（自由计时计入）。
   该表达式原先私藏在 `TaskManager` 匿名命名空间，此次上移到 `FocusSessionRules`——
   那个头文件早就写明「唯一口径，不允许在别处复制」，当时只搬了番茄口径。
   月历热力的每日值同步改为分钟：深浅表达的必须和目标进度是同一件事。
2. `FocusHistoryService` 补齐写接口：`addManualSession` / `updateSession` / `deleteSession`。
   补录记为自由计时（不伪装成番茄），拦重叠 / 未来 / 低于 3 分钟门槛，
   拒绝改动 `end_time IS NULL` 的进行中会话。
3. 今日目标条给出「已排」与差额（当天任务预计用时之和 vs 今日目标）。数据本就齐备，纯展示层。
4. `moveTaskToDate`：周计划拖动改期，落在目标日末尾。
5. `reorderTasks`：手动序写 `display_order`（从 1 起，0 表示没排过并回落到 `created_at`），
   升级后既有列表顺序不变。
6. `tasks.notes`：添加/编辑弹窗可写，任务行单行省略显示；
   `updateTask` 的备注用 null 表示「保持不变」、空串表示「清空」。

**踩到并记下的三个坑**（都不是表面问题）：
- **Qt 的 SQLite 驱动不支持重名具名占位符**。`INSERT` 里 `:date` 出现两次（写值 +
  子查询算序号），只绑第一处，整条语句失败。子查询里的日期必须另取名字。
- **空 `QString` 是 null**，绑进去写成 `NULL`，撞 `NOT NULL` 约束。需要写空串时要显式收敛。
- **测试里的日期也必须走逻辑日**。`tst_today_goal_card` 里有一条用例用了 `new Date()` 的
  物理日期，而视图按 `dayStartHour=4` 算——白天跑碰巧过，凌晨 4 点前必红。

真实库上已核对迁移结果：`user_version` 10→11，既有目标 100 番茄 → 2500 分钟，
90 条任务一条不少，该科目累计 4336 分钟（此前对目标零贡献）全部计入。

**下面第 299/309/719 行提到的 `estimated_pomodoros`、以及各处的「番茄数」目标口径，
都是当时的写法，现已不存在。**

**历史状态基线：分支 `feature/keyboard-shortcuts` 的 `e3a38b8`（2026-08-08 复核）。**
**已无待执行计划。** 001–024、029、031 全部 DONE（009 之后被产品决定回退）；
025/027/028 于 2026-08-08 实测后 REJECTED，026 需重写，030 降级为写法整洁项。

**2026-08-16：第七轮质量与安全审计，基于 commit `f8e3120`。**
新增 035–040，严格按维护者确认的顺序执行。当前全量基线为 **63/63，15.97s**
（`QT_QPA_PLATFORM=offscreen QT_QUICK_CONTROLS_STYLE=Basic`，Qt 6.10.3）。
本轮只生成计划，没有修改生产代码。

**2026-09-01：全局主动休息与超长自由专注修正。**
042 已实施在 `feature/new-requirement`：主动休息入口位于今日任务页“添加任务”左侧，独立于
...
设置弹窗的半提交（当时只确认输入预校验；2026-09-04/05 又补齐真实持久化失败时的补偿回滚与缓存恢复，见顶部最新进度）。
...
| 032 | 给数据库连接设置 busy_timeout，导出期间主线程写入不再瞬间失败 | P1 | S | — | DONE（`796596f`，已证伪：timeout 改回 0 时新用例转红） |
| 033 | 导出进行中挡住退出与关窗，把 exportService.busy 接进界面 | P1 | S | — | DONE（`796596f`，判据抽成 ShutdownGuard 纯函数并证伪） |
| 034 | 45 个 QML 测试拆成独立 ctest 条目，让 `ctest -j` 真正并行 | P2 | S | — | DONE（`f2aff68` + 后续去 RUN_SERIAL，全量 57s → 16s） |
| 035 | 原子切换目标详情，禁止弹窗标题与删除主键分裂 | P1 | S | — | DONE（`5d60952`，隔离 worktree 复核：63/63 passed；旧实现新增用例稳定转红） |
| 036 | 恢复数据库期间统一阻断快捷键与直接动作分发 | P1 | S | — | DONE（`5c91af7`，隔离 worktree 复核：63/63 passed；旧实现全局热键用例稳定转红） |
| 037 | 统一导出关闭语义，并让“全部导出”读取同一数据库快照 | P1 | M | 032、033 | DONE（`96f0e84`，隔离 worktree 复核：63/63 passed；旧按钮与旧双读取结构均稳定转红） |
| 038 | 新恢复点提交成功后再清理旧快照 | P1 | S | — | DONE（`4d22b70`，隔离 worktree 复核：63/63 passed；旧实现会把 3 份恢复点提前删成 2 份） |
| 039 | 删除恒真断言，并把 qmllint 整类豁免收紧为逐点基线 | P1 | S | — | DONE（`c1445f3`，隔离 worktree 复核：63/63 passed；门禁 mutation 以 exit 8 稳定转红） |
| 040 | 定义并统一任务排序不变量，消除 0/正序号和写路径分裂 | P2 | M | — | DONE（`e5c8b26`，隔离 worktree 复核：63/63 passed；迁移、追加写入、完整集校验三类 mutation 均稳定转红） |
| 042 | 全局主动休息与超长自由专注时长修正 | P1 | M | — | DONE（`44f7b70`、`47a130f`；已纳入当前分支） |
| 043 | 待办课表页与数据完整性修复 | P1 | M | — | DONE（含 2026-09-05 边界修复；70/70 通过） |
| 044 | QML 运行时告警门禁 | P1 | S | — | DONE（`daeac91`；当前覆盖 51 个 QML 测试条目） |

### 035–040 依赖与排序说明

- 035、036 独立，先处理直接误删和数据库替换临界区。
- 037 依赖已完成的 032（数据库 busy_timeout）与 033（导出 busy/退出守卫）；没有这两层，
  共享读事务和 busy 弹窗会建立在错误前提上。
- 038、039 无代码依赖，按已确认优先级排在导出之后。
- 040 技术上独立，但必须最后执行：它包含 schema v12 和所有任务写路径，风险高于前五个小改动。

### 042 依赖与排序说明

- 042 不依赖审计计划。它新增与 `BreakPhase` 分离的 `ManualRestPhase`，必须继续保持
  `FocusLiveSeconds` 对非专注阶段的排除逻辑；应在当前 `feature/new-requirement` 分支独立维护。

### 2026-08-16 明确不纳入本轮的审计项

- **备份 Schema/数据域全量校验**：进入 backlog，不建立编号计划。当前威胁模型是单机单用户、
  用户主动导入；完整白名单与旧 Schema（尤其 `long_goals` 懒建表）兼容风险高，工作量 L。
  若产品开始互传备份文件或接收非受信来源，再提升优先级。
- **备份资源预算**：证据成立，但维护者未独立跑过，也未排进本轮执行序列。后续单独定义文件大小、
  行数、字段长度与耗时上限，不能混进 Schema 白名单计划。
- **倒计时 model reset**：证据接受但未独立复现，本轮不排期。开工前先补稳定复现和边界测试。

`Export All` 的共享快照已并入 037；不是 backlog。
...
## 第六轮审计（2026-08-14，针对 v10/v11 与异步导出之后的新代码）

### 范围与理由（第六轮）

上一次审计（第五轮 + 2026-08-08 的实测复核）之后，代码变动很大：schema v10/v11、
深色主题 palette 全线接管、CVE-2025-12385 的 textFormat 收敛、Qt 升到 6.10.3、
外部审计的 SEC/PERF 七条、导出移出 GUI 线程。本轮重点审**这些新代码**，
而不是重扫全仓。

**审计基线**：commit `b5a8836` **加上当时未提交的 22 个文件（+639 行）**。
按工作区实际状态审——那才是真正会跑起来的代码。这批未提交改动同时意味着
032 有一条基线前提检查（见该计划开头）。

### 已确认并立项（→ plans/032-034）

| 发现 | 证据 | 计划 |
|---|---|---|
| 导出移出 GUI 线程后，工作线程对同一库另开一条连接，但主连接没设 `busy_timeout`、库也不是 WAL。SQLite 默认 timeout 为 0，撞锁即刻返回 `SQLITE_BUSY`——导出期间主线程的一次提交（结束番茄、存会话）会当场失败，且只有 `qWarning` | `DatabaseManager.cpp:115-119` 只设 `foreign_keys`；`ExportService.cpp:504-523` 开第二条连接；`BackupOperations.cpp:400` 已有 `busy_timeout=5000` 的先例，说明是遗漏 | 032 |
| `exportService.busy` 加了属性和信号，但整个 `qml/` **零消费**：关窗/退出只挡 `backupService`，导出中退出会静默放弃导出，并让工作线程与单例析构竞争 | `main.qml:126,181` 只判 backupService；`grep exportService.busy qml/` 无命中；`ExportDialog.qml:38` 导出中也能 Esc 关掉 | 033 |
| 44 个 QML 测试文件挤在一条 ctest 条目里串行跑，占 `-j4` 全量 57.05s 中的 56.16s | `CMakeLists.txt:726-729`。**实测**：44 个文件各起一个 runner + `xargs -P8` = **24.8s**；最慢单文件 `tst_ui_optimization.qml` **20.17s**（含 4.97s 写死 `wait()`） | 034 |

### 本轮**实测退役**的长期积压项（不要再携带）

- **「统计页一次刷新约 40 次全表扫描」——建议就此关闭。**
  这条从第一轮挂到现在，每轮都标「仍然有效」。本轮第一次真正验证：
  `EXPLAIN QUERY PLAN` 确认 `date(start_time, shift) = ?` 走 `SCAN`、
  改成裸列范围才走 `SEARCH ... USING INDEX idx_sessions_start`——**机制属实**。
  但代价实测：2 万行时单次扫描约 **1.8ms**，30 次合计 **53ms**；
  而真实库里是 **60 条 focus_sessions、99 个任务、两个月数据**。
  为 50ms（还是 5 年后的 50ms）去做一次会动到所有统计口径的 M 工作量重构，
  性价比不成立。**除非用户数据量涨到万级，否则不要再立项。**

### 已考虑并驳回（第六轮）

- **「文档里的测试规模数字又漂了」—— 驳回，数字是对的。**
  `README.md:69` 说「约 77 秒」，实测 `ctest -j4` 是 57s——看着像漂移，
  但 77s 正是**串行**总和（各条目耗时相加 ≈ 77.9s），文档没错，是我一开始用了 `-j4`。
  「551 条断言函数」同样成立：runner 实际报 558 passed = 462 个 `test_*`
  + 44 个文件 × 2 个 fixture(init/cleanupTestCase)。**两条都不是发现。**
- **「迁移备份 `pomodoro_backup_*.db` 永不清理」—— 不成立。**
  `DatabaseManager::pruneOldBackups`（`:1317-1330`）保留最近 3 份，
  实际目录里正好 3 个文件、共 836K。
- **「`before-restore-*` 快照永不清理」（第一轮遗留）—— 已修复。**
  `BackupService::pruneBeforeRestoreBackups`（`:965-969`）已存在。
- **「没开 `-Wall -Wextra`」（第一轮遗留）—— 已修复。** `CMakeLists.txt:231-242`。

### 未立项但已核实（下轮参考）

| 发现 | 为什么这轮没做 |
|---|---|
| 无 CI，但仓库**有** GitHub remote（`origin` → github.com）。19 条 ctest、`-Wall -Wextra`、`QmlLintGate`、`QmlTextFormatGate` 全靠开发者记得本地跑 | M 工作量（macOS runner + aqtinstall 装 Qt 6.10.3，单次约 10–15 分钟）。第一轮说「加 CI 需要先确认是否打算推 GitHub」——**这个前提现在已经成立**，可以立项了。建议排在 034 之后（拆完条目 CI 才跑得快） |
| `tst_ui_optimization.qml` 一个文件 20.17s，全仓 QML 测试共 220 处 `wait()`、合计 15.7s 写死睡眠 | 拆分（034）之后它才会成为瓶颈，那时再做才有意义 |
| `tests/ServiceTests.cpp` 已涨到 **5774 行**（第一轮记录是 4059） | L 工作量，拆分前置条件（`friend class ServiceTests`）未变，结论同第一轮 |

### 本轮未审计的范围（第六轮）

- macOS 平台层 `.mm`——第五轮已完整读过 671 行并判定干净，本轮未重读。
- QML 渲染性能——**仍未实测**（连续第六轮）。2026-08-08 的复核已把由此衍生的
  四份计划全部驳回，所以这块目前没有悬空的推断，但也确实没有数据。
- v1→v11 迁移链本身未重读（v10/v11 是上一轮刚加的）。
- 那批未提交改动的**业务逻辑**未逐条复审；只对其中新增的断言做了证伪
  （发现并修正了一条假通过的测试，见下）。

### 034 的后续：RUN_SERIAL 已撤销，根因修掉了

拆条目后 `ui_optimization` 在 `ctest -j8` 下撞满 120s watchdog（单独跑只要 20s），
当时先用 `RUN_SERIAL` 兜住。随后查清根因并撤销了它：

- **紧的 `tryCompare` 上限就是那个根因。** 它们一旦条件成立就立即返回，
  所以把上限从 120–220ms 放宽到 3000ms **在通过时不花任何时间**，
  只在真失败时多等一会儿。写紧没有任何收益，只会在 CPU 被抢占时
  把"渲染慢了"误判成"行为错了"。全仓 12 个文件共 74 处已放宽。
- **`wait(N)` 换成 `tryVerify` 等条件成立。** `tst_ui_optimization` 的 `init()`
  原本每个用例睡 220ms，28 个用例光 init 就 6.1 秒，睡够了还不保证动画到位。
- 效果：全量 57s → **16s**（`-j8`，63 条条目全绿），且
  `ui_optimization` 在 7 路 CPU 满载下实测 15.69s，与空载 15.60s 持平——
  负载敏感性消失了，不是被 RUN_SERIAL 藏起来。
- 保留的固定 `wait()` 只剩"证明某件事没发生"那几处（粒子不叠加、初始完成态不放粒子），
  它们断言的就是时序语义，换成 tryVerify 会变成同义反复。

**踩到的坑（已写进 `docs/运行命令.md` 的 QML 测试红线）**：批量放宽超时时用了
"取末尾数字当超时"的正则，把 `tryCompare(obj, "prop", 期望值)` 这种**三参形式的期望值**
改成了 3000，一次静默改写 12 处断言。三个文件当场转红才发现。
只处理确实有第 4 个参数的调用。

### 一条工装侧的记录（值得记住）

复核那批未提交改动时，`PlatformControlTests` 里新增的
`silentInstanceConnectionsAreBoundedAndExpire` **是假通过的**：
判据「服务端此刻持有的 socket 数 ≤ 16」跑在 `QTRY_VERIFY(非空)` 之后，
第一个连接刚被 accept 就满足，把上限改成 1024 照样通过。
已改为从**客户端侧**数仍处连接态的数量，并要求在 500ms 超时回收之前收敛到 16。

更值得记的是差点导致误判的那一层：用 `sed -i.bak` 改源码、再 `mv` 还原，
`.bak` 保留的是**改动前的旧 mtime**，还原后文件比 `.o` 还旧，构建判定
「无需重编译」，**于是跑的还是被改坏的二进制**。当时据此一度以为实现有问题。
**还原源文件后必须 `touch`，并确认构建输出里出现该文件的 `Building CXX object` 行。**
