...
//   palette.windowText GroupBox 标题 / 勾选框文字 / Label 默认字色 —— 同样深灰
    ...
    // —— 热力色阶（2026-09-14 定稿，规则见 docs/业务规则.md「热力取档与专注历史月历的投入条」）——
    // 四档由浅到深按投入分钟取档（HeatmapBands.js）。**浅深两套方向相反**：
    // 浅底上投入越多越深，深底上投入越多越亮；同一条暖色轴（Lab 色相 68°）各取一段。
    // 直接套用日间那套到深底，最低档会是近白块、比高投入还抢眼。
    // 相邻两档 ΔE00 ≥ 7，由 tst_heatmap_bands 断言；调色前先跑它。
    ...
    // 低彩度暖灰在普通 / 选中 / 悬停三种格底上都 ≥ 6.7，且始终比第 1 档安静。
    readonly property color heatmapEmptyTrack: darkMode ? "#544c45" : "#d9d0c7"
