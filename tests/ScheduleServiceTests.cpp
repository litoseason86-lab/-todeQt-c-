...
private slots:
    ...
    void scheduleIdMustBeAutoGenerated_data();
    void scheduleIdMustBeAutoGenerated();
    void emptyActiveWeekIsRejected();
...
void ScheduleServiceTests::scheduleIdMustBeAutoGenerated_data()
{
    QTest::addColumn<QString>("table");
    QTest::addColumn<QString>("declaration");
    for (const QString& table : {QStringLiteral("schedule_entries"), QStringLiteral("schedule_periods")}) {
        QTest::newRow(qPrintable(table + "-int")) << table << QStringLiteral("INT PRIMARY KEY");
        QTest::newRow(qPrintable(table + "-desc")) << table << QStringLiteral("INTEGER PRIMARY KEY DESC");
        QTest::newRow(qPrintable(table + "-without-rowid")) << table << QStringLiteral("WITHOUT ROWID");
        QTest::newRow(qPrintable(table + "-composite")) << table << QStringLiteral("INTEGER, PRIMARY KEY(id, start_minutes)");
    }
}

void ScheduleServiceTests::scheduleIdMustBeAutoGenerated()
{
    QFETCH(QString, table);
    QFETCH(QString, declaration);
    QTemporaryDir dir;
    const QString path = dir.filePath(QStringLiteral("bad-id.sqlite"));
    QVERIFY(DatabaseManager::instance()->initialize(path));
    QSqlQuery query(DatabaseManager::instance()->database());
    ...
    query.finish();
    // 复合主键约束必须放在全部列声明之后，保持构造出的 SQL 本身合法。
    if (declaration == QStringLiteral("WITHOUT ROWID")) {
        sql.replace(QStringLiteral("INTEGER PRIMARY KEY AUTOINCREMENT"), QStringLiteral("INTEGER PRIMARY KEY"));
        sql = sql.trimmed() + QStringLiteral(" WITHOUT ROWID");
    } else if (declaration.contains(QLatin1Char(','))) {
        sql.replace(QStringLiteral("INTEGER PRIMARY KEY AUTOINCREMENT"), QStringLiteral("INTEGER"));
        sql.insert(sql.lastIndexOf(QLatin1Char(')')), QStringLiteral(", PRIMARY KEY(id, start_minutes)"));
    } else {
        sql.replace(QStringLiteral("INTEGER PRIMARY KEY AUTOINCREMENT"), declaration);
    }
    QVERIFY(query.exec(QStringLiteral("DROP TABLE ") + table));
    QVERIFY2(query.exec(sql), qPrintable(query.lastError().text()));
    QVERIFY(!DatabaseManager::instance()->initialize(path));
}

void ScheduleServiceTests::emptyActiveWeekIsRejected()
{
    auto* service = ScheduleService::instance();
    EntryArgs original;
    QVERIFY(addEntry(original));
    const QVariantList before = service->getEntries();
    const int id = before.first().toMap().value(QStringLiteral("id")).toInt();
    QSignalSpy changed(service, &ScheduleService::scheduleChanged);
    for (int week : {1, 2}) {
        const int parity = week == 1 ? ScheduleService::EvenWeeks : ScheduleService::OddWeeks;
        EntryArgs invalid;
        invalid.weekStart = week;
        invalid.weekEnd = week;
        invalid.weekParity = parity;
        QVERIFY(!addEntry(invalid));
        QVERIFY(!service->updateEntry(id, original.title, original.weekday,
            original.startMinutes, original.endMinutes, original.location, original.categoryId,
            week, week, parity));
    }
    QCOMPARE(changed.count(), 0);
    QCOMPARE(service->getEntries(), before);
    // 与范围匹配的单周、双周以及跨两周规则仍然允许保存。
    for (int week : {1, 2}) {
        EntryArgs valid;
        valid.weekStart = week;
        valid.weekEnd = week;
        valid.weekParity = week == 1 ? ScheduleService::OddWeeks : ScheduleService::EvenWeeks;
        QVERIFY(addEntry(valid));
    }
}
