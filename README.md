...
- **任务管理**：今日任务清单、本周计划、每日例行（可编辑、跨天自动生成）、逾期结转、分类管理。
  ...
  本周计划每天给出「已排 N」与本周合计，已完成的沉到当天末尾；列表支持键盘操作
  （↑↓ 跨日移动并跳过空日、空格完成、⏎ 开始专注（仅今天）、E 编辑、⌘⌫ 删除）。
  任务与目标弹窗的科目下拉末尾有「+ 新建科目…」，不必中途跑去设置里建；
  弹窗开着时科目在别处增删改，下拉会跟着刷新并保住已选的科目（被删了就回到「不设置科目」）。
...
- **专注计时**：番茄工作法（工作—休息循环，支持长休息）与自由正向计时；沉浸模式；自由计时超过设定小时数后结束需确认记录或丢弃。
  专注页空闲时任务标题本身就是选择器：点开列出今日任务（已完成的排在末尾但仍可选），也能直接敲一个新任务并开始。
  ⌘↩ 与全局热键在空闲时按同一口径直接开始——已有选中项优先，否则取今日第一个未完成任务；
  一条都取不到时展开选择器，不会把人送到一个做不了事的页面。
...
- **专注记录可修改**：忘记开计时、中途强退、计时器绑错任务，都能在专注历史页补录、修改或删除。
  ...
  也拒绝改动正在进行的会话。
  专注历史的月历格底有一条投入条，按当天专注分钟分四档着色（< 60 / 60–149 / 150–299 / ≥ 300），
  不用逐格读时长就能看出哪天垮了；没学的日子留一条灰轨道，还没到的日子不画条。
...
- **知识缺口**：专注时发现「这块没搞懂」「这个问题得专门坐下来想」，在专注页右上角一句话记下来，
  或直接按 ⌘⇧K（另有一个出厂留空的全局热键，应用在后台时也能记）。
...
- **长期目标与奖励机制**：目标以「投入分钟」计量（与任务预计用时同一把尺子，输入组件也是同一个），
  ...
  月历热力（与专注历史同一套四档分钟阈值；今天、没到的日子分别标出，读屏报「投入 N 分钟 / 无投入 / 今日暂无投入 / 尚未到来」）、按近两周自然日节奏预测完成日期（样本不足不预测），学习日另列；跨 25/50/75/100% 触发全局 Toast、里程碑弹窗、粒子与音效
...
- **快捷键**：20 个动作全部可自定义（设置中心「快捷键」分页录制改键、冲突检测、单行/整体恢复默认、可停用）。
  应用内 16 个开箱即用：⌘1–⌘8 切页、⌘N 新建任务、⌘⇧K 记一笔、⌘↩ 开始/暂停专注、⌘⇧↩ 结束专注、
  ⌘⇧F 沉浸模式、⌘\ 侧栏、⌘, 设置、⌘/ 快捷键速查。应用内键位**可以只用一个键**（空格、数字键等）：
  焦点在输入框时无修饰键的快捷键自动让路，带 ⌘/⌃/⌥ 的照常可用；弹窗接管焦点时整体让路，不会在弹窗背后误切页。
  另有 4 个全局热键（开始/暂停、结束专注、召回/隐藏窗口、记一笔）走 Carbon `RegisterEventHotKey`，
  应用在后台也能触发且**不需要辅助功能授权**——但**出厂不占用任何系统按键**，
  需要时在设置里自行指定（预设键位难免和已装的其他应用撞车，而撞车表现是别的应用那个键失灵，很难排查）。
  **切页键只覆盖 8 个页面，侧栏现有 12 个入口**（11 个可排序页面 + 固定在底部的「设置」）——「课表」「今日专注」「知识缺口」没有独立快捷键，可通过侧栏点击或 Tab 聚焦后回车进入。
