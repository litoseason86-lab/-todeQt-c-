...
**Goal:** 引入可配置逻辑日界点（默认凌晨 4 点，0–6 整点）：AppSettings.dayStartHour、LogicalDay 纯函数库（C++/JS）、**所有 focus_sessions SQL 分桶入口**同口径改逻辑日、RoutineManager 按逻辑今天生成例行、LogicalDayService 失效信号（构造即排期 + 跨日补例行）、StatisticsView 时间源改逻辑今天并订阅失效、设置弹窗步进器。
...
**批次边界修正：** 规格影响面汇总把 `RoutineManager` 列在计划二，但计划一已经建立 `changed → materializeToday` 接线并承诺“跨日自动补例行”。若计划一仍连接物理日实现，这个交付就是假完成。因此 `RoutineManager::materializeToday()` 的逻辑日改造随接线一并放入计划一；计划二不再重复修改它。
...
void ServiceTests::logicalDayDateOfBoundaries()
{
    ...
    // today 是 dateOf(now) 的薄包装。用调用前后两个时刻包围结果，
    // 即使测试恰好跨过日界点，也不会因两次独立取时制造竞态假失败。
...
**Interfaces:**
- Consumes: `LogicalDay::msUntilNextBoundary/today`、`AppSettings::dayStartHour/dayStartHourChanged`。
- Produces: `LogicalDayService::instance()`、信号 `changed()`；边界 QTimer objectName `"logicalDayBoundaryTimer"`（测试契约）；`RoutineManager::materializeToday()` 按逻辑今天落任务与 `last_generated_date`；上下文属性 `logicalDayService`；main.cpp 里 `changed → RoutineManager::materializeToday` 直连。
...
{
    // 从 0-6 中选距下一界点最远的一项，排除测试执行途中恰好跨界的竞态。
    ...
    // 直接发统一失效信号，精确复现 main.cpp 接线，不通过改设置偷带日期变化。
    service.changed();
    ...
    // 同一逻辑日再次失效 → materializeToday 幂等，不重复插。
    service.changed();
    QCOMPARE(countRoutineTasks(), 1);
}
```

定时器只断言 `isActive()`：`remainingTime()` 在恰好到期、事件循环尚未派发 timeout 的瞬间允许为 0，断言 `> 0` 会制造日界点附近的竞态假失败；具体间隔已由 Task 2 的 `msUntilNextBoundary` 纯函数测试覆盖。
...
- [ ] **Step 5: RoutineManager 的 materializeToday 改逻辑今天 + 既有测试口径修正**

`src/services/RoutineManager.cpp` include 区加：
...
`materializeToday()`（约 255 行）把物理今天：
...
```cpp
    // 例行任务属于逻辑日：凌晨日界点前生成时仍落在前一天，避免 0 点后重复生成。
    const QString today = LogicalDay::today(
                              AppSettings::instance()->dayStartHour()).toString(Qt::ISODate);
```

`tests/ServiceTests.cpp` 中以下既有函数把“服务的今天”写成了 `QDate::currentDate()`；实现切换后凌晨运行会假失败。在这些函数体内把相应调用改为计划一 Task 3 已提供的 `logicalToday()`：
...
```bash
awk '/^void ServiceTests::/{fn=$2} /QDate::currentDate/{print NR" "fn}' tests/ServiceTests.cpp | grep -E "materializeToday"
```
...
Run: `cmake --build build && QT_QPA_PLATFORM=offscreen ./build/PomodoroTodoTests logicalDayServiceSchedulesTimerOnConstruction logicalDayServiceEmitsChangedOnDayStartHourChange logicalDayChangeMaterializesRoutineIdempotently materializeTodayIsIdempotentAndDoesNotBackfill materializeTodayPreservesCategoryAndDoesNotEmitSignals materializeTodayStampsRoutineId materializeTodayRollsBackClaimWhenTaskInsertFails materializeTodayDoesNotResurrectDeletedTask materializeTodaySkipsInactiveRoutines`
Expected: 9 passed；失效接线测试同时断言任务日期与 `last_generated_date` 都是逻辑今天，不能只数任务数量。

- [ ] **Step 8: 提交**

```bash
git add src/services/LogicalDayService.h src/services/LogicalDayService.cpp src/services/RoutineManager.cpp CMakeLists.txt src/main.cpp tests/ServiceTests.cpp
git commit -m "例行生成改逻辑今天并新增 LogicalDayService 失效排期与补齐接线"
```
...
TestCase {
    ...
    // StatisticsView 有两个未限定名 Connections；测试必须提供 signal-only mock，
    // 否则组件创建时会出现 ReferenceError，日期断言即使通过也不可信。
    QtObject {
        id: taskManager
...
汇报：专注归日全口径统一（Statistics 4 处 + FocusHistory 命名重构 + Export，`grep date(.*start_time` 零残留）、RoutineManager 按逻辑今天生成、LogicalDayService 构造即排期 + 例行补齐接线、统计页时间源=逻辑今天 + 订阅失效、设置可调 0-6。等待用户确认后进入计划二（任务/倒计时/周计划/QML 今天）。**不自行合并回 main。**
