...
3. 进入方式：**手动按钮**——专注页右上角 ⛶。（原与铃铛并排；铃声快捷开关后被整体移除，见组件一注。）
...
1. **⛶ 按钮**：右上角，沿用原 `soundToggleButton` 的幽灵按钮风格与尺寸。（实现定稿：铃声快捷开关从专注页移除、统一由设置对话框控制，⛶ 顶替其右上角位置，见提交 b2127d0。）
   - 显示条件：**计时进行中（含暂停）**——
...
- 右上角浮现组：✕ 退出全屏。（实现阶段定稿：原设计的 🔔 声音快捷开关移除——铃声统一由设置对话框控制，沉浸层保持极简，见提交 81ca7c8、b2127d0。）
